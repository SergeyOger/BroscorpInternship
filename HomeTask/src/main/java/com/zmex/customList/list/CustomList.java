...
public class CustomList<T extends Number> implements Serializable, Cloneable {
  ...
  public <R extends Number> CustomList map(Function<T, R> function) {
    CustomList<R> resultList = new CustomList<>();
    Arrays.stream(DATA_ARRAY)
        .limit(elementCounter)
        .map(x -> function.apply((T) x))
